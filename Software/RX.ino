...
const char* ssid     = "ESP32 Alpha Centauri";
...
void loop() {
  ...
  if (packetSize) {
    ...
    while (LoRa.available()) {
      ...
      String outputVoltage = LoRaData.substring(indicador1 + 1, indicador2);
      Serial.println(outputVoltage);

      int indicador3 = LoRaData.indexOf(',', indicador2 + 1);
      String uvIntensity = LoRaData.substring(indicador2 + 1, indicador3);
      Serial.println(uvIntensity);

      int indicador4 = LoRaData.indexOf(',', indicador3 + 1);
      String baseline = LoRaData.substring(indicador3 + 1, indicador4);
      Serial.println(baseline);

      int indicador5 = LoRaData.indexOf(',', indicador4 + 1);
      String P = LoRaData.substring(indicador4 + 1, indicador5);
      Serial.println(P);

      int indicador6 = LoRaData.indexOf(',', indicador5 + 1);
      String A = LoRaData.substring(indicador5 + 1, indicador6);
      Serial.println(A);

      int indicador7 = LoRaData.indexOf(',', indicador6 + 1);
      String T = LoRaData.substring(indicador6 + 1, indicador7);
      Serial.println(T);



      /*int indicador6 = LoRaData.indexOf(',', indicador5);
      String P = LoRaData.substring(indicador5 + 1, indicador6);
      Serial.print("Presion Absoluta: ");
      Serial.print(float(P) * 0.01, 2);
      Serial.print(" hpa | ");
      Serial.print(float(P) * 0.750063755);
      Serial.print(" mmHg | ");
      Serial.print(float(P) * 0.014503774);
      Serial.println(" psi");*/
    }
